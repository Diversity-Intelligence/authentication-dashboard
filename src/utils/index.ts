...
import { FactorIds, HTTPStatusCodes, StorageKeys } from 'consts';
import { getAccessDeniedEvent } from 'event';
import { NetworkManager, localStorageHandler } from 'services';
...
export function getApiUrl(path: string, tenantId?: string): string {
  ...
  if (tenantId !== undefined) {
    dashboardBasePathToUse = dashboardBasePathToUse.replace(
      '/dashboard',
      `/${tenantId}/dashboard`,
    );
  }
...
export const isUsingDemoConnectionUri = (connectionURI: string) => {
  return DEMO_CONNECTION_URIS.some((domains) =>
    connectionURI.includes(domains),
  );
};
...
export const useFetchData = (skipTriggeringErrorBoundary = false) => {
  ...
  const fetchData = async ({
    ...
    const logoutAndRedirect =
      shouldRedirectOnUnauthorised &&
      HTTPStatusCodes.UNAUTHORIZED === response.status;
...
export const formatLongDate = (date: number | Date) => {
  ...
  }${yearToDisplay}${delimiter}
${(hour % 12 || 12).toString().padStart(2, '0')}:${date
    .getMinutes()
    .toString()
...
export const doesTenantHasPasswordlessEnabled = (
  tenantFirstFactors: string[],
): boolean => {
  return (
