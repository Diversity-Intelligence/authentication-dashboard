...
export type ToastNotificationContextItem =
  PopupContextItem<ToastNotificationProps>;
...
export const PopupContentContext = createContext<PopupContentContextProps>(
  POPUP_CONTENT_CONTEXT_DEFAULT_VALUE,
);

export const PopupContentContextProvider = ({
  children,
}: PropsWithChildren) => {
  ...
  const removeToast = (itemId: number) =>
    dispatchToast({ type: 'remove', itemId });
  ...
  const removeModal = (itemId: number) =>
    dispatchModal({ type: 'remove', itemId });

  return (
    <PopupContentContext.Provider
      value={{ toasts, showToast, removeToast, modals, showModal, removeModal }}
    >
      {children}
    </PopupContentContext.Provider>
  );
