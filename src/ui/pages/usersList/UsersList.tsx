...
import { localStorageHandler } from 'services';
...
import {
  ...
  useAppEnvContext,
} from '../../../ui/contexts/AppEnvContext';
...
type UserListPropsReloadRef = MutableRefObject<
...
export const UsersList: React.FC<UserListProps> = ({
  ...
  const [paginationTokenByOffset, setPaginationTokenByOffset] =
    useState<NextPaginationTokenByOffset>({});
  ...
  const insertUsersAtOffset = useCallback(
    (paramUsers: User[], paramOffset?: number, isSearch?: boolean) => {
      ...
      return [
        ...users.slice(0, paramOffset),
        ...paramUsers,
        ...users.slice(paramOffset + limit),
      ];
    },
    [users, limit],
  );
  ...
  const getOffsetByPaginationToken = useCallback(
    (paginationToken?: string) => {
      ...
      const matchedPaginationTokenByOffsetPair = Object.entries(
        paginationTokenByOffset,
      ).find(([_, token]) => paginationToken === token);
  ...
  const loadUsers = useCallback(
    async (paginationToken?: string, search?: object) => {
      ...
      if (paginationToken !== undefined) {
        data = await fetchUsers({ paginationToken }, undefined, tenantId).catch(
          () => undefined,
        );
        setIsSearch(false);
      } else if (search === undefined || Object.keys(search).length === 0) {
        data = await fetchUsers(undefined, undefined, tenantId).catch(
          () => undefined,
        );
        setIsSearch(false);
      } else {
        data = await fetchUsers({ limit: 1000 }, search, tenantId).catch(
          () => undefined,
        );
        setIsSearch(true);
        localSearch = true;
      ...
      if (data) {
        ...
        setPaginationTokenByOffset({
          ...paginationTokenByOffset,
          [nextOffset]: nextPaginationToken,
        });
  ...
  const fetchAndSetCurrentTenant = async () => {
    ...
    if (tenantInStorage === undefined) {
      ...
      const filteredTenants = result.tenants.filter(
        (t) => t.tenantId === tenantInStorage,
      );
  ...
  return (
    ...
    >
      ...
      {tenantsListFromStore !== undefined &&
        tenantsListFromStore.length > 1 && (
      ...
      <div className='search-container'>
        ...
        <Button
          disabled={
            selectedTenant === undefined || tenantsListFromStore === undefined
          }
          id='add-user'
          className='ml-auto'
          color='secondary'
...
export const UserListPage = () => {
  ...
  const [selectedUserEmailVerification, setSelectedUserEmailVerification] =
    useState<EmailVerificationStatus | undefined>();
  ...
  const { sendUserEmailVerification: sendUserEmailVerificationApi } =
    useVerifyUserTokenService();
  ...
  const onUserDelete = useCallback(
    async (userId: string) => {
      ...
      const didSucceed =
        deleteSucceed !== undefined && deleteSucceed.status === 'OK';
      if (didSucceed) {
  ...
  const updateEmailVerificationStatus = useCallback(
    async (
      ...
      const isUpdated = await updateUserEmailVerificationStatus(
        userId,
        isVerified,
        tenantId,
      );
      ...
      showToast(
        isVerified
          ? getEmailVerifiedToast(isUpdated)
          : getEmailUnVerifiedToast(isUpdated),
      );
      return isUpdated;
    },
    [showToast],
  ...
  useEffect(() => {
    if (
      currentLocation &&
      currentLocation.search.includes('userid') === false
    ) {
      backToList();
    }
  }, [currentLocation.search]);
        ...
        <UserDetail
          ...
          onSendEmailVerificationCallback={({ id, tenantIds }) => {
            return sendUserEmailVerification(
              id,
              tenantIds.length > 0 ? tenantIds[0] : undefined,
            );
          }}
