...
export default function CreateUserDialog({
  ...
  const [currentStep, setCurrentStep] = useState<CreateUserDialogStepType>(
    'select-auth-method-and-tenant',
  );
  ...
  const selectedTenantObject = tenants.find(
    ...
    if (selectedAuthMethod === 'passwordless') {
      return (
        <Alert
          ...
          type='primary'
        >
          Custom API overrides for the <code>consumeCodePOST</code> API won't
          run as the API to create a user via the dashboard is different.
          However, custom functions override for <code>consumeCode</code> will
          run.
        </Alert>
      );
    }
    ...
    if (selectedAuthMethod === 'emailpassword') {
      return (
        ...
        >
          <ul>
            <li>
              Custom API overrides for the sign up post API won't be run as the
              API to create a user via the dashboard is different. However,
              custom functions override for sign up will run.
            </li>{' '}
            <li>
              If you have custom form fields in your sign up form, those will
              not be included when adding a user via the dashboard. If you want
              to include any custom form fields in the request, please call the{' '}
              <a
        ...
        <div className='create-user-modal-content'>
          ...
          <div className='select-container mb-28'>
            ...
            {selectableAuthMethods.length >= 1 ? (
              <Select
                onOptionSelect={(value) =>
                  setSelectedAuthMethod(value as AuthMethod)
                }
              ...
              <div className='input-field-error block-small block-error w-100'>
                <p className='text-xs text-command'>
                  Please configure passwordless or the emailpassword recipe on
                  the backend SDK and this tenant. Refer to the{' '}
                  <a
