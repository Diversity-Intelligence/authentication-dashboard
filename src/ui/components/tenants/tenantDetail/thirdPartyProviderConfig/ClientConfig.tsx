...
import { trash as TrashIcon } from 'assets';
...
export const ClientConfig = ({
  ...
  const handleAdditionalConfigChange = (
    ...
    let found = false;
    const newAdditionalConfig: [string, string | null][] =
      client.additionalConfig.map(([k, v]) => {
        if (k === key) {
          found = true;
        ...
        <div className='client-config-container__fields'>
          ...
          {additionalConfigFields?.map((field) => (
            <ThirdPartyProviderInput
              ...
              minLabelWidth={LABEL_MIN_WIDTH}
              error={
                errors[`clients.${clientIndex}.additionalConfig.${field.id}`]
              }
              forceShowError
