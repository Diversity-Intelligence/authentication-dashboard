...
import { SAML_PROVIDER_ID } from 'consts';
...
export const ProviderInfoFormForBoxy = ({
  ...
  const [samlInputType, setSamlInputType] = useState<'xml' | 'url' | 'manual'>(
    () => {
      ...
      return (
        (providerConfigState.clients![0]?.additionalConfig?.samlInputType as
          | 'xml'
          | 'url'
          | 'manual') || 'manual'
      );
        ...
        clients: prev.clients!.map((client, index) =>
          index === 0
            ? {
                ...
                additionalConfig: {
                  ...client.additionalConfig,
                  [samlInputType === 'xml' ? 'samlXML' : 'samlURL']:
                    e.target.value,
                },
              }
            : client,
  ...
  const handleSave = async () => {
    ...
    if (samlInputType === 'xml' || samlInputType === 'url') {
      const redirectURLs = (providerConfigState.clients![0].additionalConfig
      ...
      if (filteredRedirectURLs.length === 0) {
        setErrorState((prev) => ({
          ...prev,
          redirectURLs: ['At least one redirect URL is required'],
        }));
        isValid = false;
      } else {
      ...
      if (
        ...
        setErrorState((prev) => ({
          ...prev,
          samlURL: 'A valid SAML URL is required',
        }));
        isValid = false;
      }
    }
    ...
    const normalizedProviderConfig = {
      ...
      clients: providerConfigState.clients!.map((client) => ({
        ...
        additionalConfig: {
          ...
          samlXML:
            samlInputType === 'xml'
              ? client.additionalConfig?.samlXML
              : undefined,
          samlURL:
            samlInputType === 'url'
              ? client.additionalConfig?.samlURL
              : undefined,
        },
      })),
    };
        ...
        {samlInputType === 'manual' && (
          <>
            <p className='saml-info-note'>
              ...
              to learn about Boxy integration and obtaining Client ID and
              Secret.
            </p>
            ...
            <ThirdPartyProviderInput
              ...
              handleChange={(e) => {
                setProviderConfigState((prev) => ({
                  ...
                  clients: prev.clients!.map((client, index) =>
                    index === 0
                      ? { ...client, clientSecret: e.target.value }
                      : client,
                  ),
                }));
              }}
            ...
            <ThirdPartyProviderInput
              ...
              isRequired={true}
              value={
                providerConfigState.clients![0].additionalConfig?.boxyURL || ''
              }
              handleChange={(e) => {
                setProviderConfigState((prev) => ({
                  ...prev,
    ...
    <div className='redirect-urls-container'>
      ...
      {redirectURLs?.slice(1).map((redirectURL, index) => (
        ...
        >
          ...
          <DeleteCrossButton
            onClick={() =>
              setRedirectURLs(redirectURLs.filter((_, i) => i !== index + 1))
            }
            label='Delete URL'
          />
        </div>
