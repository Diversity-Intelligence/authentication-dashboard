...
import { IN_BUILT_THIRD_PARTY_PROVIDERS, SAML_PROVIDER_ID } from 'consts';
...
export const ProviderInfoForm = ({
  ...
  const [providerConfigState, setProviderConfigState] = useState(
    getInitialProviderInfo(providerConfig, providerId),
  );
  ...
  const createOrUpdateThirdPartyProvider =
    useCreateOrUpdateThirdPartyProviderService();
  ...
  const baseProviderId = isSAMLProvider
    ? SAML_PROVIDER_ID
    : (inBuiltProviderInfo?.id ?? '');
  const shouldUsePrefixField =
    isAddingNewProvider && (inBuiltProviderInfo || isSAMLProvider);
  const customFieldProviderKey = Object.keys(
    IN_BUILT_PROVIDERS_CUSTOM_FIELDS_FOR_CLIENT,
  ).find((id) => providerId?.startsWith(id));
  ...
  const handleAddNewClient = () => {
    setProviderConfigState((prev) => {
      const commonFields: Pick<
        ProviderClientState,
        'additionalConfig' | 'forcePKCE' | 'scope'
      > = {
        scope: [''],
        additionalConfig: [['', '']],
        forcePKCE: false,
      ...
      if (prev.clients[0]) {
        ...
        commonFields.additionalConfig = Array.isArray(additionalConfig)
          ? [...additionalConfig]
          : [['', '']];
      ...
      if (customFields) {
        commonFields.additionalConfig = customFields.map((field) => [
          field.id,
          '',
        ]);
      }
  ...
  const handleSave = async () => {
    ...
    if (providerConfigState.thirdPartyId.trim() === '') {
      setErrorState((prev) => ({
        ...prev,
        thirdPartyId: 'Third Party Id is required',
      }));
      isValid = false;
      ...
      setErrorState((prev) => ({
        ...prev,
        thirdPartyId:
          'Third Party Id can only contain lowercase alphabets, numbers and hyphens',
      }));
      isValid = false;
    ...
    providerConfigState.clients?.forEach((client, index) => {
      if (
        ...
        setErrorState((prev) => ({
          ...prev,
          [`clients.${index}.clientId`]: 'Client Id is required',
        }));
        isValid = false;
      }
      if (!isAppleProvider) {
      ...
      if ((providerConfigState.clients?.length ?? 0) > 1) {
        if (
          ...
          setErrorState((prev) => ({
            ...prev,
            [`clients.${index}.clientType`]: 'Client Type is required',
          }));
          isValid = false;
          return;
        }
    ...
    if (customFields !== undefined && customFields.length > 0) {
      providerConfigState.clients?.forEach((client, index) => {
        customFields?.forEach((field) => {
          const fieldValue = client.additionalConfig.find(
            ([key]) => key === field.id,
          )?.[1];
          if (
            field.required &&
    ...
    const normalizedProviderConfigClients = providerConfigState.clients?.map(
      (client) => {
        ...
        return {
          ...
          additionalConfig: Object.fromEntries(
            client.additionalConfig.filter(
              ([key, _]: [string, string | null]) => key.trim().length > 0,
            ),
          ),
    ...
    const normalizedProviderConfig = {
      ...
      oidcDiscoveryEndpoint:
        providerConfigState.oidcDiscoveryEndpoint.trim() || undefined,
      tokenEndpoint: providerConfigState.tokenEndpoint.trim() || undefined,
      userInfoEndpoint:
        providerConfigState.userInfoEndpoint.trim() || undefined,
      authorizationEndpoint:
        providerConfigState.authorizationEndpoint.trim() || undefined,
      ...
      userInfoMap: {
        fromIdTokenPayload: {
          userId:
            providerConfigState.userInfoMap.fromIdTokenPayload?.userId?.trim() ||
            undefined,
          email:
            providerConfigState.userInfoMap.fromIdTokenPayload?.email?.trim() ||
            undefined,
          emailVerified:
            providerConfigState.userInfoMap.fromIdTokenPayload?.emailVerified?.trim() ||
            undefined,
        },
        fromUserInfoAPI: {
          userId:
            providerConfigState.userInfoMap.fromUserInfoAPI?.userId?.trim() ||
            undefined,
          email:
            providerConfigState.userInfoMap.fromUserInfoAPI?.email?.trim() ||
            undefined,
          emailVerified:
            providerConfigState.userInfoMap.fromUserInfoAPI?.emailVerified?.trim() ||
            undefined,
        },
      },
      authorizationEndpointQueryParams:
        normalizedAuthorizationEndpointQueryParams,
    ...
    try {
      setIsSaving(true);
      await createOrUpdateThirdPartyProvider(
        tenantInfo.tenantId,
  ...
  const isKnownThirdPartyId = (thirdPartyId: string) => {
    ...
    return IN_BUILT_THIRD_PARTY_PROVIDERS.some((provider) =>
      thirdPartyId.startsWith(provider.id),
    );
  };
  ...
  return (
    <PanelRoot>
      ...
      <div className='fields-container'>
        {shouldUsePrefixField ? (
          isSuffixFieldVisible ? (
            <ThirdPartyProviderInput
              ...
              type='text'
              name='thirdPartyId'
              value={providerConfigState.thirdPartyId.slice(
                baseProviderId.length + 1,
              )}
              forceShowError
            ...
            <div className='suffix-preview-field'>
              ...
              <div className='suffix-preview-container'>
                ...
                {errorState.thirdPartyId && (
                  <div className='suffix-preview-container__error'>
                    {errorState.thirdPartyId}
                  </div>
                )}
              </div>
        ...
        <KeyValueInput
          ...
          onChange={(value) => {
            setProviderConfigState((prev) => ({
              ...prev,
              authorizationEndpointQueryParams: value,
            }));
          }}
        />
        ...
        {providerConfig?.isGetAuthorisationRedirectUrlOverridden && (
          <div className='overridden-info'>
            <b>Note:</b> You cannot edit the above fields because this provider
            is using a custom override for{' '}
            <code>getAuthorisationRedirectUrl</code>
          </div>
        )}
        ...
        {providerConfig?.isExchangeAuthCodeForOAuthTokensOverridden && (
          <div className='overridden-info'>
            <b>Note:</b> You cannot edit the above fields because this provider
            is using a custom override for{' '}
            <code>exchangeAuthCodeForOAuthTokens</code>
          </div>
        )}
        ...
        <KeyValueInput
          ...
          onChange={(value) => {
            setProviderConfigState((prev) => ({
              ...prev,
              userInfoEndpointQueryParams: value,
            }));
          }}
        />
        ...
        {emailSelectValue === 'never' && (
          <div className='overridden-info'>
            <b>Note:</b> We will generate a fake email for the end users
            automatically using their user id.
          </div>
        )}
        ...
        {providerConfig?.isGetUserInfoOverridden && (
          <div className='overridden-info'>
            <b>Note:</b> You cannot edit the above fields because this provider
            is using a custom override for <code>getUserInfo</code>
          </div>
        )}
      ...
      <div
        className={`custom-provider-footer ${formHasError ? 'custom-provider-footer--error' : ''}`}
      >
        {formHasError && (
...
const getInitialProviderInfo = (
  ...
  let additionaConfigFields: [string, string][] = customFields.map((field) => [
    field.id,
    '',
  ]);
    ...
    return {
      ...
      userInfoMap: {
        fromIdTokenPayload: {
          ...
          emailVerified:
            providerConfig.userInfoMap?.fromIdTokenPayload?.emailVerified ?? '',
        },
        fromUserInfoAPI: {
          userId: providerConfig.userInfoMap?.fromUserInfoAPI?.userId ?? '',
          email: providerConfig.userInfoMap?.fromUserInfoAPI?.email ?? '',
          emailVerified:
            providerConfig.userInfoMap?.fromUserInfoAPI?.emailVerified ?? '',
        },
      },
      clients:
        Array.isArray(providerConfig.clients) &&
        providerConfig.clients.length > 0
          ? providerConfig.clients.map((client) => ({
              ...client,
              additionalConfig:
                client.additionalConfig &&
                Object.keys(client.additionalConfig).length > 0
      ...
      tokenEndpointBodyParams:
        providerConfig.tokenEndpointBodyParams &&
        Object.keys(providerConfig.tokenEndpointBodyParams).length > 0
      ...
      userInfoEndpointQueryParams:
        ...
        Object.keys(providerConfig.userInfoEndpointQueryParams).length > 0
          ...
          : [['', '']],
      userInfoEndpointHeaders:
        providerConfig.userInfoEndpointHeaders &&
        Object.keys(providerConfig.userInfoEndpointHeaders).length > 0
