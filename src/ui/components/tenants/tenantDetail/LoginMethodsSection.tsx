...
import { FIRST_FACTOR_IDS, SECONDARY_FACTOR_IDS } from 'consts';
import { formFieldErrorIcon as ErrorIcon } from 'assets';
    ...
    <>
      <PanelRoot>
        ...
        {tenantInfo.firstFactors.length === 0 && (
          <ErrorBlock className='tenant-detail__factors-error-block'>
            At least one login method needs to be enabled for the user to log in
            to the tenant.
          </ErrorBlock>
        )}
      ...
      <PanelRoot>
        ...
        {mfaError === 'MFA_REQUIREMENTS_FOR_AUTH_OVERRIDDEN' && (
          <ErrorBlock className='tenant-detail__factors-error-block'>
            Please note that the MFA functions are overridden in the SDK and the
            required secondary factors settings will be based on the overridden
            logic.
          </ErrorBlock>
        )}
        <div className='tenant-detail__factors-container'>
          <div className='tenant-detail__factors-container__grid'>
            {SECONDARY_FACTOR_IDS.map((method) => (
              <LoginFactor
                ...
                description={method.description}
                checked={
                  tenantInfo.requiredSecondaryFactors?.includes(method.id) ??
                  false
                }
...
const LoginFactor = ({
  ...
  setMfaError?: (
    error:
      | null
      | 'MFA_NOT_INITIALIZED'
      | 'MFA_REQUIREMENTS_FOR_AUTH_OVERRIDDEN',
  ) => void;
}) => {
  ...
  const handleFactorChange = async () => {
    ...
    try {
      ...
      if (type === 'first-factor') {
        const res = await updateFirstFactors(
          tenantInfo.tenantId,
          id,
          !doesFactorExist,
        );
        ...
        const res = await updateSecondaryFactors(
          tenantInfo.tenantId,
          id,
          !doesFactorExist,
        );
  ...
  return (
    <div
      className={`tenant-detail__factors-container__grid__factor${
        fixedGap
          ? ' tenant-detail__factors-container__grid__factor--fixed-gap'
          : ''
      }`}
    >
      <div className='tenant-detail__factors-container__grid__factor__label-container'>
        ...
        >
          {hasError ? (
            <ErrorIcon
              style={{ transform: 'translateY(-1px)', width: '14px' }}
            />
          ) : (
            <InfoIcon />
          )}
        </TooltipContainer>
        <div className='tenant-detail__factors-container__grid__factor__label-container__label'>
          {label}:
        </div>
      </div>
      <Toggle
