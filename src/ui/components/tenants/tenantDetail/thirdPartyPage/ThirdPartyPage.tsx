...
const ProviderInfo = ({
  ...
  const [
    ...
    setHasFilledCustomFieldsForProvider,
  ] = useState(!PROVIDERS_WITH_ADDITIONAL_CONFIG.includes(providerId ?? ''));
  const getThirdPartyProviderInfo = useGetThirdPartyProviderInfoService();
  const [providerConfigResponse, setProviderConfigResponse] = useState<
    ProviderConfigResponse | undefined
  >(undefined);
  const providerHasCustomFields = PROVIDERS_WITH_ADDITIONAL_CONFIG.includes(
    providerId ?? '',
  );
  ...
  const fetchProviderInfo = async (
    ...
    const response = await getThirdPartyProviderInfo(
      tenantInfo.tenantId,
      id,
      additionalConfig,
    );
    if (response.status === 'OK') {
      ...
      if (
        ...
        response.providerConfig.clients![0].additionalConfig?.boxyAPIKey ===
          undefined
      ) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      (!(isAddingNewProvider && providerHasCustomFields) ||
...
const BoxySamlForm = ({
  ...
  const isBoxyUrlDisabled =
    !isAddingNewProvider && currentAdditionalConfig?.boxyURL !== undefined;
  ...
  return (
    <div className='saml-intro-container'>
      ...
      <SAMLInfoBox title='Managed Service'>
        <a href='mailto:team@supertokens.com'>Email us</a> to receive your Boxy
        URL and continue setup of your SAML client.
      </SAMLInfoBox>
      ...
      <p className='saml-intro-container__boxy-url-header'>
...
const GoogleWorkspacesForm = ({
  ...
  return (
    <div className='additional-config-container'>
      ...
      <p className='additional-config-container__note'>
        For example: use "example.com" if you want to allow logins only from
        that domain. Enter “ * “ if you want to allow logins for any google
        workspace domain.
      </p>
...
const PROVIDERS_WITH_ADDITIONAL_CONFIG = [
  'google-workspaces',
  'active-directory',
  'okta',
  'boxy-saml',
