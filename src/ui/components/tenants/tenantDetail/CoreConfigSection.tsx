...
import { PUBLIC_TENANT_ID } from 'consts';
import { editUnfilled as PencilIcon } from 'assets';
import { infoIcon as InfoIcon } from 'assets';
import { questionMark as QuestionMarkIcon } from 'assets';
...
export const CoreConfigSection = () => {
  ...
  const hasPluginProperties = tenantInfo.coreConfig.some(
    (config) => config.isPluginProperty,
  );
  ...
  if (hasPluginProperties) {
    if (
      tenantInfo.coreConfig.some((property) =>
        property.key.startsWith('postgresql_'),
      )
    ) {
      databaseType = 'postgres';
    } else if (
      tenantInfo.coreConfig.some((property) =>
        property.key.startsWith('mysql_'),
      )
    ) {
      databaseType = 'mysql';
    }
    ...
    <PanelRoot>
      ...
      <div className='tenant-detail__core-config-table'>
        <div className='tenant-detail__core-config-table__header'>
          <div className='tenant-detail__core-config-table__header__item'>
            Property name
          </div>
          <div className='tenant-detail__core-config-table__header__item'>
            Value
        ...
        <>
          <div className='tenant-detail__core-config-table__plugin-properties-container'>
            ...
            <p className='tenant-detail__core-config-table__plugin-properties-description'>
              Some of these properties need to be modified together, hence they
              cannot be directly modified from the UI, instead you can make an
              API request to core to modify these properties.{' '}
              <button
...
const CoreConfigTableRow = ({
  ...
  const [
    ...
    setIsUneditablePropertyDialogVisible,
  ] = useState(false);
  const [isEditPropertyDialogVisible, setIsEditPropertyDialogVisible] =
    useState(false);
      ...
      if (isUsingPublicApp) {
        if (isPublicTenant) {
          ...
          return (
            <>
              Please use the{' '}
              <a href='https://supertokens.com/dashboard-saas'>
                SuperTokens SaaS Dashboard
              </a>{' '}
        ...
        if (isPublicTenant) {
          ...
          return (
            <>
              Please use the Update App API to configure this property. Refer to
              the{' '}
    ...
    <>
      ...
      {isUneditablePropertyDialogVisible && (
        <UneditablePropertyDialog
          onCloseDialog={() => setIsUneditablePropertyDialogVisible(false)}
        >
