...
export const isEmailVerificationApplicable = (
  recipeId: string,
...
export const NameTooltip: FC<{ fieldName: string }> = ({ fieldName }) => (
  <>
    <p>
      This information is fetched using the{' '}
      <span className='block-snippet-small'>{fieldName}</span> key from the meta
      data set for this user.
      <br />
      <br />
      To change this information, please add / change{' '}
      <span className='block-snippet-small'>{fieldName}</span> key using our
      usermetadata feature.
    </p>
  </>
);
...
export const UserDetailProviderBox: FC<{ user: User }> = ({ user }) => {
  ...
  const useLogoIcon = ['apple', 'github', 'google', 'facebook'].includes(
    id.toLowerCase(),
  );

  return (
    <div
      className={`user-detail__provider-box block-snippet ${id.toLowerCase()}`}
    >
      <span>
        {
...
export const UserDetailInfoGridItem: FC<UserDetailInfoGridItemProps> = ({
  label,
  body,
  tooltip,
}) => {
...
export const EmailVerifiedField: FC<EmailVerifiedFieldProps> = (
  props: EmailVerifiedFieldProps,
) => {
  const { user, isEditing, setVerificationStatus, sendVerification } = props;

  const isApplicable = isEmailVerificationApplicable(
    user.loginMethods[0].recipeId,
    user.emails[0],
  );
...
export const UserDetailInfoGrid: FC<UserDetailInfoGridProps> = (props) => {
  ...
  const { showLoadingOverlay, hideLoadingOverlay, userDetail } =
    useUserDetailContext();
  const [emailErrorFromAPI, setEmailErrorFromAPI] = useState<
    string | undefined
  >(undefined);
  const [phoneErrorFromAPI, setPhoneErrorFromAPI] = useState<
    string | undefined
  >(undefined);
  ...
  if (
    userDetail.metaData !== undefined &&
    userDetail.metaData !== METADATA_NOT_ENABLED_TEXT
  ) {
  ...
  const onSave = useCallback(async () => {
    showLoadingOverlay();
    const response = await userDetail.func.updateUser(
      userDetail.userId,
      userState,
