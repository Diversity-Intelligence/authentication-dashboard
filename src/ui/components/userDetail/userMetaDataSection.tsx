...
export const UserMetaDataSection: React.FC = () => {
  const { hideLoadingOverlay, showLoadingOverlay, userDetail } =
    useUserDetailContext();
  const [isEditing, setIsEditing] = useState<boolean>(false);
  const [metadataForEditing, setMetaDataForEditing] = useState(
    userDetail.metaData,
  );
  const [metaDataUpdateError, setMetaDataUpdateError] = useState<
    string | undefined
  >(undefined);
  ...
  const renderMetaDataContent = () => {
    ...
    const highlightedCode = HighlightJS.highlight(
      getFormattedMetaData(userDetail.metaData),
      {
        language: 'typescript',
      },
    );
  ...
  const onSave = async () => {
    ...
    try {
      ...
      await updateUserMetaData(
        userDetail.userId,
        metadataForEditing === undefined || metadataForEditing === ''
          ? '{}'
          : metadataForEditing,
      );
