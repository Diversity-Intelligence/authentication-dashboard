...
type UserUnlinkConfirmationProps = {
  onConfirmed: (isConfirmed: boolean) => void;
  loginMethod: LoginMethod;
};
...
export type UserDetailChangePasswordPopupProps = Omit<
  LayoutModalProps,
  'modalContent'
> & {
  userId: string;
...
export type UserDetailChangeEmailPopupProps = Omit<
  LayoutModalProps,
  'modalContent'
> & {
  userId: string;
...
export type UserDetailChangePhonePopupProps = Omit<
  LayoutModalProps,
  'modalContent'
> & {
  userId: string;
...
export const getUserChangeEmailPopupProps = (
  props: UserDetailChangeEmailPopupProps,
) => {
  const closeModalRef: React.MutableRefObject<(() => void) | undefined> = {
    current: undefined,
  };
...
export const UserDetailChangeEmailForm: FC<UserDetailChangeEmailFormProps> = (
  ...
  const onSave = async () => {
    ...
    const tenants: Tenant[] = getTenantsObjectsForIds(
    ...
    if (recipeId === 'emailpassword') {
      matchingTenants = tenants.filter((tenant) =>
        tenant.firstFactors.includes(FactorIds.EMAILPASSWORD),
      );
    }

    if (recipeId === 'passwordless') {
      matchingTenants = tenants.filter((tenant) =>
        doesTenantHasPasswordlessEnabled(tenant.firstFactors),
      );
    }
    ...
    <>
      <div className='user-detail-form'>
        ...
        <InputField
          ...
          hideColon={true}
          error={
            repeatEmail !== undefined && !isEmailMatch
...
export const UserDetailChangePasswordForm: FC<
  UserDetailChangePasswordFormProps
> = (props: UserDetailChangePasswordFormProps) => {
  ...
  const onSave = async () => {
    ...
    const tenants = getTenantsObjectsForIds(
      tenantsListFromStore ?? [],
      props.tenantIds,
    );
    const matchingTenantIds = tenants.filter((_tenant) =>
      _tenant.firstFactors.includes(FactorIds.EMAILPASSWORD),
    );
    ...
    try {
      const response = await updatePassword(
        userId,
        password,
        matchingTenantIds.length > 0
          ? matchingTenantIds[0].tenantId
          : undefined,
      );
    ...
    <>
      <div className='user-detail-form'>
        ...
        <InputField
          ...
          hideColon={true}
          error={
            repeatPassword !== undefined && !isPasswordMatch
...
export const LoginMethodUnlinkConfirmation: FC<UserUnlinkConfirmationProps> = ({
  onConfirmed,
  loginMethod,
}) => {
  ...
  if (loginMethod.recipeId === 'passwordless') {
    informationToEnter =
      loginMethod.phoneNumber !== undefined
  ...
  return (
    <div className='user-detail-form'>
      <p>
        Are you sure you want to unlink the selected Login method{' '}
        <span>{loginMethod.recipeId}</span>?
      </p>
      <p>
        To unlink the user, please confirm by typing the {inputType}:{' '}
...
export const LoginMethodDeleteConfirmation: FC<
  UserDeleteConfirmationProps & { loginMethod: LoginMethod }
> = ({ user, onConfirmed, loginMethod }) => {
  const [inputValue, setInputValue] = useState<string>('');
  const [showError, shouldShowError] = useState(false);
  ...
  return (
    <div className='user-detail-form'>
      ...
      <p>
        Please confirm by typing the {inputType}:{' '}
        <span>{informationToEnter}</span> below
      </p>
      <div className='user-delete-input-container'>
        <InputField
...
export const UserDeleteConfirmation: FC<UserDeleteConfirmationProps> = ({
  user,
  onConfirmed,
  loginMethod,
}) => {
  ...
  if (
    user.loginMethods[0].recipeId === 'passwordless' &&
    user.phoneNumbers[0] !== undefined
  ) {
  ...
  if (loginMethod !== undefined) {
    if (loginMethod.email !== undefined) {
      ...
      inputType = loginMethod.phoneNumber
        ? "user's phone number"
        : "user's email id";
    }
  }
  ...
  return (
    <div className='user-detail-form'>
      <p>
        To delete the user, please confirm by typing the {inputType}:{' '}
        <span>{informationToEnter}</span> below
      </p>
      <p>{''}</p>
...
export const getUserDeleteConfirmationProps = (
  props: UserDeleteConfirmationTriggerProps,
) => {
  const { user, onDeleteCallback, loginMethod, all } = props;
  const closeConfirmDeleteRef: React.MutableRefObject<
    (() => void) | undefined
  > = { current: undefined };
...
export const getLoginMethodUnlinkConfirmationProps = (
  props: UnlinkLoginMethodProps,
) => {
...
export const getDeleteUserToast = (isSuccessfull: boolean) => {
  return {
    iconImage: getImageUrl(
      isSuccessfull ? 'trash.svg' : 'form-field-error-icon.svg',
    ),
    toastType: 'error',
    children: (
      <>{isSuccessfull ? 'User deleted successfully' : 'User is not deleted'}</>
    ),
...
export const getUnlinkUserToast = (isSuccessfull: boolean) => {
  return {
    ...
    children: (
      <>
        {isSuccessfull ? 'User unlinked successfully' : 'User is not unlinked'}
      </>
    ),
  } as ToastNotificationProps;
};
...
export const getUpdateUserToast = (isSuccessfull: boolean) => {
  return {
    iconImage: getImageUrl(
      isSuccessfull ? 'checkmark.svg' : 'form-field-error-icon.svg',
    ),
...
export const getSendEmailVerificationToast = (isSuccessfull: boolean) => {
  return {
    iconImage: getImageUrl(
      isSuccessfull ? 'envelope-green.svg' : 'form-field-error-icon.svg',
    ),
...
export const getEmailVerifiedToast = (isSuccessfull: boolean) => {
  return {
    iconImage: getImageUrl(
      isSuccessfull ? 'checkmark-green.svg' : 'form-field-error-icon.svg',
    ),
    toastType: isSuccessfull ? 'success' : 'error',
    children: (
      <>
        {isSuccessfull
          ...
          : 'Email is failed to set as verified'}
      </>
    ),
  } as ToastNotificationProps;
};
...
export const getEmailUnVerifiedToast = (isSuccessfull: boolean) => {
  return {
    iconImage: getImageUrl(
      isSuccessfull ? 'people-restricted.svg' : 'form-field-error-icon.svg',
    ),
    toastType: isSuccessfull ? 'info' : 'error',
    children: (
      <>
        {isSuccessfull
          ? 'Email set as unverified'
          : 'Email is failed to set as unverified'}
      </>
    ),
  } as ToastNotificationProps;
};
...
export const getUpdatePasswordToast = (isSuccessfull: boolean) => {
  return {
    ...
    toastType: isSuccessfull ? 'success' : 'error',
    children: (
      <>{isSuccessfull ? 'Password updated' : 'Failed to update password'}</>
    ),
  } as ToastNotificationProps;
};
