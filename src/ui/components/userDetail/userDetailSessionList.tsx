...
export const UserDetailsSessionList: React.FC = () => {
  ...
  const sessionCountText =
    sessionList === undefined
      ? ''
      : `(TOTAL NO OF SESSIONS: ${sessionList.length})`;
  ...
  const revokeAllSessions = async () => {
    ...
    try {
      ...
      const allSessionHandles: string[] = sessionList.map(
        (item) => item.sessionHandle,
      );
      ...
      <div className='content-container'>
        <div className='header'>
          <span className='header-primary'>
            SESSION INFORMATION{' '}
            <span className='header-secondary'>{sessionCountText}</span>
          </span>
          <div
