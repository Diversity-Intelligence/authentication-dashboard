...
import useUserService, {
  type IUpdateUserInformationArgs,
...
const ProviderPill = ({ id, userId }: { id: string; userId: string }) => {
  ...
  const trim = (val: string) => {
    ...
    return (
      val.substring(0, Math.floor(len / 7)) +
      '...' +
      val.substring(6 * Math.floor(len / 7), len)
    );
  };
...
const UserRecipePill = (loginMethod: LoginMethod) => {
  return (
    <div
      className={`pill ${loginMethod.recipeId} ${loginMethod.thirdParty?.id ?? ''}`}
    >
...
type MethodProps = {
  ...
  onEditCallback: (
    val: IUpdateUserInformationArgs,
        ...
        <div className='left'>
          ...
          <span className='user-id-span'>
            ...
            <span className='copy-text-wrapper'>
              <CopyText copyVal={loginMethod.recipeUserId}>
                {loginMethod.recipeUserId}
              </CopyText>
            </span>
          </span>
            ...
            <div>
              {loginMethod.thirdParty && (
                <>
                  ...
                  <ProviderPill {...loginMethod.thirdParty} />
                </>
              )}
            </div>
          </>
...
export const LoginMethods: React.FC<LoginMethodProps> = ({
  refetchAllData,
  refetchUsersList,
}) => {
  ...
  const onEditCallback = useCallback(
    async (vals: IUpdateUserInformationArgs) => {
    ...
    >
      {methods.map((val, ind) => (
        <Methods
          loginMethod={val}
          onUpdateEmailVerificationStatusCallback={
            userDetail.func.onUpdateEmailVerificationStatusCallback
          }
          key={ind}
          ...
          showUnlink={
            methods.length > 1 ||
            (userDetail.details.isPrimaryUser === true && methods.length === 1)
          }
        />
      ))}
    </LayoutPanel>
