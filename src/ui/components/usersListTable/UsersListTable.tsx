...
export type UserRowActionProps = Pick<
  UserDetailProps,
  'onDeleteCallback' | 'onChangePasswordCallback'
>;
...
const UserInfo = ({ user }: { user: User }) => {
  ...
  const methodFilter = user.loginMethods.filter(
    (el) => el.recipeUserId === user.id,
  );
  const email =
    methodFilter.length > 0 ? methodFilter[0].email : user.emails[0];
  const phone =
    methodFilter.length > 0
      ? methodFilter[0].phoneNumber
      : user.phoneNumbers[0];
...
export const UserRecipePill = ({ user }: { user: User }) => {
  ...
  const recipeId =
    loginMethods.length > 1 ? 'multiple' : loginMethods[0].recipeId;
  return (
    <RecipePill
      recipeId={recipeId}
...
const UserDate = ({ user }: { user: User }) => {
  return (
    <div className='user-date'>
      {user.timeJoined && formatLongDate(user.timeJoined)}
    </div>
  );
};
...
const UserTablePaginationInfo = (
  props: Pick<UserListProps, 'count' | 'limit' | 'offset' | 'users'>,
) => {
  ...
  return (
    <p className='users-list-pagination-count text-small'>
      ...
      {formatNumber(Math.min(offset + displayedLength, count))} of{' '}
      {formatNumber(count)}
    </p>
  );
};
...
export const PlaceholderTableRows = (props: {
  rowCount: number;
  colSpan: number;
  className?: string;
}) => {
...
const getEstimatedCount = (
  props: Pick<
    UserListProps,
    'count' | 'limit' | 'users' | 'nextPaginationToken'
  >,
) => {
  const { count } = {
    ...props,
