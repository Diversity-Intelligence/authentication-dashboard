...
import { greenCheck as GreenCheckIcon } from 'assets';
import { loader as LoaderIcon } from 'assets';
import { noResults as NoResultsIcon } from 'assets';
import { plusSquare as PlusAdd } from 'assets';
...
export default function AssignRolesDialog({
  ...
  const [roleToAssign, setRoleToAssign] = useState<string | undefined>(
    undefined,
  );
  ...
  function renderRoles() {
    ...
    if (roles.length === assignedRoles.length && roles.length > 0) {
      return (
        <div className='info-container'>
          ...
          <p>
            <Link to='/roles'>Click here</Link> to create more user roles that
            you require!
          </p>
        </div>
      );
    ...
    if (roles.length < 1) {
      return (
        <div className='info-container'>
          ...
          <p>
            <Link to='/roles'>Click here</Link> to create roles that you can
            assign to users
          </p>
        </div>
      );
    ...
    const rolesNotAssigned = roles.filter(
      (r) => assignedRoles.includes(r) === false,
    );
    const filteredRolesBySearch =
      searchText !== ''
        ? rolesNotAssigned?.filter((role) =>
