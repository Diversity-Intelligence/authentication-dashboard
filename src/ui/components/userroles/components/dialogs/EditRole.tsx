...
export default function EditRoleDialog({
  ...
  const [isDeletePermissionsDialogOpen, setIsDeletePermissionsDialogOpen] =
    useState(false);
    ...
    try {
      ...
      const updatedRolesData = roles.map((role) => {
        if (role.role === currentlySelectedRole.role) {
          role.permissions = [
            ...currentlySelectedRole.permissions,
            ...newlyAddedPermissions,
          ];
        }
        return role;
              ...
              <div>
                <TagsInputField
                  addTag={(permision: string) => {
                    if (
                      currentlySelectedRole.permissions.includes(permision) ===
                        false &&
                  ...
                  removeTag={(permission) => {
                    setNewlyAddedPermissions(
                      newlyAddedPermissions.filter((p) => p !== permission),
                    );
                  }}
