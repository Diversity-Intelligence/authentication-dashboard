...
export const useListTenantsService = () => {
  ...
  const fetchTenants = async (): Promise<{
    ...
    result.tenants.sort((a: Tenant, b: Tenant) =>
      (a.tenantId === 'public' ? '' : a.tenantId).localeCompare(
        b.tenantId === 'public' ? '' : b.tenantId,
      ),
    );
...
export const useCreateTenantService = () => {
  ...
  const createTenant = async (
    ...
    | {
        status:
          | 'MULTITENANCY_NOT_ENABLED_IN_CORE_ERROR'
          | 'TENANT_ID_ALREADY_EXISTS_ERROR';
      }
    | {
...
export const useGetThirdPartyProviderInfoService = () => {
  ...
  > => {
    const additionalConfigQueryParams = new URLSearchParams(
      additionalConfig,
    ).toString();
